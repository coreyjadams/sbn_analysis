...
NueAppearanceFitter::NueAppearanceFitter() {
  ...
  uboonePhotonScale = 1.0;
  LAr1NDPhotonScale = 1.0;
  icarusPhotonScale = 1.0;
...
int NueAppearanceFitter::Prepare() {
  ...
  if (use100m) {
    ...
    dirtScales.push_back(LAr1NDDirtScale);
    photonScales.push_back(LAr1NDPhotonScale);
  ...
  if (use470m) {
    ...
    dirtScales.push_back(ubooneDirtScale);
    photonScales.push_back(uboonePhotonScale);
  ...
  if (useT600_onaxis) {
    ...
    dirtScales.push_back(icarusDirtScale);
    photonScales.push_back(icarusPhotonScale);
...
int NueAppearanceFitter::ReadData() {
  ...
  for (int b_line = 0; b_line < nL; b_line ++) {
    for (int bin = 0; bin < nbins_nue; bin++) {
      ...
      if (photonScales[b_line] != 1.0) {
        NueFromNC_pi0Vec[b_line][bin]           *= scales[b_line];
        NueFromNC_delta0Vec[b_line][bin]        *= scales[b_line];
        eventsnueVec[b_line][bin] -= NueFromNC_pi0Vec[b_line][bin];
        NueFromNC_pi0Vec[b_line][bin]           *= photonScales[b_line];
        NueFromNC_delta0Vec[b_line][bin]        *= photonScales[b_line];
        eventsnueVec[b_line][bin] += NueFromNC_pi0Vec[b_line][bin];
      }
      else {
        NueFromNC_pi0Vec[b_line][bin]             *= scales[b_line];
      ...
      DirtVec[b_line][bin]                      *= scales[b_line] * dirtScales[b_line];
...
int NueAppearanceFitter::Loop() {
  ...
  for (unsigned int bin = 0; bin < nullVec.size(); bin++)
  {
    ...
    if (inflateSystematics) {
      // std::cout << "Bin " << bin << " entry was "
                // << (*fullCovarianceMatrix)[bin][bin] << std::endl;
      (*fullCovarianceMatrix)[bin][bin] +=
        nullVec.at(bin) * systematicInflationAmount * nullVec.at(bin) * systematicInflationAmount;
      // std::cout << "It now is " << (*fullCovarianceMatrix)[bin][bin] << std::endl;
    }
  }
...
int NueAppearanceFitter::MakeSimplePlot() {
  ...
  TLegend * leg4 = plotUtils.getMiniBooneLegend();
  TLegend * leg5 = plotUtils.getMiniBooneLegend();
  // leg3->Draw();
  ...
  leg4->AddEntry(gdummy4, "Global Best Fit (arXiv:1303.3011)", "P*");
  leg4->AddEntry(gdummy0, "Global Fit 90% CL (arXiv:1303.3011)");
